...
class _LeaveScreenState extends State<LeaveScreen> {
  ...
  int usedDays = 0;
  int remainingDays = 0;
  String _filter = 'all'; // all|pending|approved|rejected|cancel_requested
  ...
  Future<void> load() async {
    ...
    items = await api
        .get('/api/leave-requests${uid.isEmpty ? '' : '?userId=$uid'}');
  ...
  Future<void> submit(String signature) async {
    ...
    if (!re.hasMatch(startCtrl.text) || !re.hasMatch(endCtrl.text)) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('날짜 형식은 YYYY-MM-DD 입니다.')));
      }
      return;
    }
  ...
  Future<void> _cancelLeaveDirect(String id) async {
    try {
      if (id.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context)
              .showSnackBar(const SnackBar(content: Text('항목 ID가 유효하지 않습니다')));
        }
        return;
      }
      final ok = await _confirm('정말로 취소하시겠습니까?');
      if (!ok) return;
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        if (mounted)
          ScaffoldMessenger.of(context)
              .showSnackBar(const SnackBar(content: Text('취소되었습니다')));
      } else {
  ...
  Future<void> _requestCancelApproved(String id) async {
    try {
      final ok = await _confirm('승인 된 연차입니다. 정말로 취소하시겠습니까?');
      if (!ok) return;
      await api.post('/api/leave-requests/' + id + '/cancel-request', {});
      await load();
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('취소신청 되었습니다')));
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('취소신청 실패: $e')));
    }
  }
      ...
      child: Column(children: [
        ...
        Row(children: [
          ElevatedButton(
              onPressed: () {
                ...
                startCtrl.text = d.toIso8601String().substring(0, 10);
                setState(() {});
              },
              child: const Text('시작=오늘')),
        ...
        TextField(
          ...
          decoration: const InputDecoration(
              ...
              hintText: '시작일 YYYY-MM-DD',
              suffixIcon: Icon(Icons.calendar_today)),
          onTap: () async {
            final now = DateTime.now();
            final initial = () {
              try {
                return DateTime.parse(startCtrl.text);
              } catch (_) {
                return now;
              }
            }();
        ...
        Row(children: [
          ElevatedButton(
              onPressed: () {
                final d = DateTime.now();
                endCtrl.text = d.toIso8601String().substring(0, 10);
                setState(() {});
              },
              child: const Text('종료=오늘')),
          const SizedBox(width: 8),
          ElevatedButton(
              onPressed: () {
                final d = DateTime.now().add(const Duration(days: 1));
        ...
        TextField(
          ...
          decoration: const InputDecoration(
              border: OutlineInputBorder(),
              hintText: '종료일 YYYY-MM-DD',
              suffixIcon: Icon(Icons.calendar_today)),
          onTap: () async {
            final now = DateTime.now();
            final initial = () {
              try {
                return DateTime.parse(endCtrl.text);
              } catch (_) {
                return now;
              }
            }();
        ...
        TextField(
            ...
            decoration: const InputDecoration(
                border: OutlineInputBorder(), hintText: '개인사유')),
        const SizedBox(height: 8),
        ...
        Wrap(spacing: 8, runSpacing: 4, children: [
          ...
          ChoiceChip(
              label: const Text('취소요청'),
              selected: _filter == 'cancel_requested',
              onSelected: (_) {
                setState(() => _filter = 'cancel_requested');
              }),
        ]),
        const SizedBox(height: 8),
          ...
          child: Builder(
            builder: (_) {
              final filtered = items.where((raw) {
                ...
                final st = (it['state'] ?? '') as String;
                final cs = (it['cancelState'] ?? 'none') as String;
                switch (_filter) {
                  case 'pending':
                    return st == 'pending';
                  case 'approved':
                    return st == 'approved' && cs != 'requested';
                  case 'rejected':
                    return st == 'rejected';
              ...
              return ListView.builder(
                ...
                itemBuilder: (c, i) {
                  ...
                  final isMine =
                      (ApiSession.userId ?? '') == (it['userId'] ?? '');
                  final state = (it['state'] ?? '') as String?;
                  final cancelState = (it['cancelState'] ?? 'none') as String;
                  final statusText = _stateKo(state) +
                      (cancelState == 'requested' ? ' (취소요청)' : '');
                  final sigUrl = (it['signature'] ?? '') as String;
                  return ListTile(
                    title: Text(
                        "${(it['userName'] ?? it['userId'])} / ${_rangeWithDays((it['startDate'] ?? '') as String, (it['endDate'] ?? '') as String)} / $statusText"),
                    subtitle: Text((it['reason'] ?? '') as String),
                    trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                      if (sigUrl.isNotEmpty)
                        Padding(
                          padding: const EdgeInsets.only(right: 8),
                          child: Image.network(ApiClient().base + sigUrl,
                              width: 56, height: 40, fit: BoxFit.contain),
                        ),
                      if (isMine && cancelState != 'requested')
                        if (state == 'approved')
                          IconButton(
                            ...
                            onPressed: () => _requestCancelApproved(
                                (it['id'] ?? '').toString()),
                          )
                        else
                          IconButton(
                            ...
                            onPressed: () =>
                                _cancelLeaveDirect((it['id'] ?? '').toString()),
                          ),
                    ]),
                    onTap: () {
                      ...
                      showDialog(
                          ...
                          builder: (_) => AlertDialog(
                              content:
                                  Image.network(ApiClient().base + sigUrl)));
                    },
                  );
                },
              );
...
class _SignatureDialogState extends State<_SignatureDialog> {
  ...
  Future<void> _onConfirm() async {
    ...
    if (bytes == null) {
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('이미지 생성 실패')));
      return;
    }
    ...
    try {
      ...
      final res =
          await widget.api.post('/api/uploads/base64', {'data': dataUrl});
