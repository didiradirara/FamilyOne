...
class _AnnouncementsScreenState extends State<AnnouncementsScreen> {
  ...
  void initState() {
    ...
    load();
    try {
      RealtimeStore.I.clear('announcements');
    } catch (_) {}
  }
  ...
  Future<void> submit() async {
    ...
    await api.post('/api/announcements', {
      ...
      if (teamDetail != null && teamDetail!.isNotEmpty)
  ...
  Future<void> markRead(String id) async {
    ...
    try {
      await api.post('/api/announcements/$id/read', {'userId': uid});
      await load();
    } catch (_) {}
  }
  ...
  Future<void> loadTeams(String siteKey) async {
    try {
      final list =
          await api.get('/api/org/teams?site=$siteKey') as List<dynamic>;
                ...
                onPressed: () async {
                  try {
                    ...
                    for (final x in picked) {
                      ...
                      final dataUrl = 'data:image/' +
                          (isPng ? 'png' : 'jpeg') +
                          ';base64,' +
                          base64Encode(bytes);
                      try {
                        final up = await api.post('/api/uploads/base64',
                            {'data': dataUrl, 'filename': x.name});
                                ...
                                onPressed: () async {
                                  try {
                                    final list = await api.get(
                                            '/api/announcements/${it['id']}/unread')
                                        as List<dynamic>;
                                    if (!mounted) return;
                                    showDialog(
                                        context: context,
