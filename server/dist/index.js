...
function maintLog(line) {
    try {
        fs.appendFileSync(path.join(logsDir, 'uploads-maintenance.log'), `[${new Date().toISOString()}] ${line}\n`);
    }
    catch { }
}
...
function cleanupUploads() {
    try {
        ...
        const rows = sqlite.prepare('SELECT imagesJson FROM reports').all();
        const used = new Set();
        for (const r of rows) {
            if (!r?.imagesJson)
                continue;
            ...
            try {
                ...
                if (now - st.mtimeMs > ttlMs) {
                    try {
                        fs.unlinkSync(fp);
                        removed++;
                        maintLog(`CLEAN OK ${name}`);
                    }
                    catch (e) {
                        maintLog(`CLEAN FAIL ${name} ${e?.message || ''}`);
                    }
                }
            }
            catch { }
        ...
        maintLog(`CLEAN ERROR ${e?.message || ''}`);
    }
}
