...
function auditLog(line) {
    try {
        fs.appendFileSync(path.join(logsDir, 'uploads-audit.log'), `[${new Date().toISOString()}] ${line}\n`);
    }
    catch { }
}
...
apiRouter.patch('/reports/:id', requireRole('manager', 'admin'), (req, res) => {
    ...
    if (existing.site && req.auth?.site && existing.site !== req.auth.site)
        return res.status(403).json({ error: 'Forbidden: cross-site' });
    if (parsed.success) {
        ...
        const r = repo.addReportImages(req.params.id, parsedImgs.data.addImages);
        if (!r)
            return res.sendStatus(404);
        notify('report:updated', r);
        return res.json(r);
        ...
        const toRemove = parsedRem.data.removeImages;
            ...
            for (const url of toRemove) {
                ...
                if (rel && !rel.startsWith('..') && !path.isAbsolute(rel)) {
                    try {
                        ...
                        auditLog(`DELETE OK ${name}`);
                    }
                    catch (e) {
                        auditLog(`DELETE FAIL ${name} ${e?.message || ''}`);
                    }
                }
            }
        }
        catch (e) {
            auditLog(`DELETE LOOP FAIL ${e?.message || ''}`);
        }
        if (!r)
            return res.sendStatus(404);
        notify('report:updated', r);
...
apiRouter.patch('/reports/:id/self', (req, res) => {
    ...
    if (pMsg.success) {
        ...
        const r = repo.addReportImages(req.params.id, pAdd.data.addImages);
        return res.json(r);
    }
    else if (pRem.success) {
        const r = repo.removeReportImages(req.params.id, pRem.data.removeImages);
        return res.json(r);
    }
    return res.status(400).json({ error: 'Invalid payload' });
});
    ...
    try {
        ...
        for (const url of images) {
            ...
            if (rel && !rel.startsWith('..') && !path.isAbsolute(rel)) {
                try {
                    fs.unlinkSync(filePath);
                    auditLog?.(`DELETE FILE OK ${name}`);
                }
                catch (e) {
                    auditLog?.(`DELETE FILE FAIL ${name} ${e?.message || ''}`);
                }
            }
        }
    }
...
apiRouter.post('/requests', (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    const creator = repo.findUserById(parsed.data.createdBy);
    const it = repo.createRequest({ ...parsed.data, site: creator?.site, team: creator?.team, teamDetail: creator?.teamDetail });
...
apiRouter.patch('/requests/:id/approve', requireRole('manager', 'admin'), (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    const existing = repo.getRequestById(req.params.id);
    if (!existing)
        return res.sendStatus(404);
    if (existing.site && req.auth?.site && existing.site !== req.auth.site)
        return res.status(403).json({ error: 'Forbidden: cross-site' });
...
apiRouter.post('/announcements', requireRole('manager', 'admin'), (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    const creator = repo.findUserById(parsed.data.createdBy);
    const ann = repo.createAnnouncement({ ...parsed.data, site: creator?.site, team: creator?.team, teamDetail: creator?.teamDetail });
...
apiRouter.post('/schedule', requireRole('manager', 'admin'), (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    const sh = repo.createShift(parsed.data);
...
apiRouter.patch('/schedule/:id', requireRole('manager', 'admin'), (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    const sh = repo.updateShift(req.params.id, parsed.data);
    if (!sh)
        return res.sendStatus(404);
    res.json(sh);
});
...
apiRouter.patch('/org/team/:id', requireRole('admin'), (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid payload' });
    const t = repo.updateTeam(req.params.id, parsed.data);
    if (!t)
        return res.sendStatus(404);
    res.json(t);
});
