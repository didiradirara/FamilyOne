...
import type { Role, Report, RequestItem, Announcement, ChecklistSubmission, Suggestion, LeaveRequest } from './types.js';
...
export const repo = {
  ...
  findUserById(id: string) {
    return sqlite.prepare('SELECT id,name,role FROM users WHERE id = ?').get(id) as any;
  },
  findUserByName(name: string) {
    return sqlite.prepare('SELECT id,name,role FROM users WHERE name = ?').get(name) as any;
  },

  // Reports
  createReport(data: { type: string; message: string; createdBy: string }): Report {
    const id = uuid();
    ...
    return { id, type: data.type as any, message: data.message, createdAt: now, createdBy: data.createdBy, status: 'new' };
  },
  listReports(): Report[] {
    return sqlite.prepare('SELECT * FROM reports ORDER BY createdAt DESC').all() as any;
  },
  updateReportStatus(id: string, status: 'new'|'ack'|'resolved') {
    sqlite.prepare('UPDATE reports SET status = ? WHERE id = ?').run(status, id);
    return sqlite.prepare('SELECT * FROM reports WHERE id = ?').get(id) as any;
  },

  // Requests
  createRequest(data: { kind: string; details: string; createdBy: string }): RequestItem {
    ...
    return { id, kind: data.kind as any, details: data.details, createdAt: now, createdBy: data.createdBy, state: 'pending' } as any;
  },
  listRequests(): RequestItem[] { return sqlite.prepare('SELECT * FROM requests ORDER BY createdAt DESC').all() as any; },
  ...
  // Announcements
  createAnnouncement(data: { title: string; body: string; createdBy: string }): Announcement {
    const id = uuid(); const now = new Date().toISOString();
    ...
    return { id, title: data.title, body: data.body, createdAt: now, createdBy: data.createdBy, readBy: [] } as any;
  },
  listAnnouncements(): Announcement[] {
    const rows = sqlite.prepare('SELECT * FROM announcements ORDER BY createdAt DESC').all() as any[];
