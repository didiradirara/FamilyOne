...
apiRouter.post('/reports', (req, res) => {
  ...
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: 'Invalid payload' });
  const report = repo.createReport(parsed.data as any);
...
apiRouter.post('/requests', (req, res) => {
  ...
  const it = repo.createRequest(parsed.data as any); notify('request:new', it); res.status(201).json(it);
});
...
apiRouter.post('/announcements', requireRole('manager','admin'), (req, res) => {
  ...
  const parsed = schema.safeParse(req.body); if (!parsed.success) return res.status(400).json({ error: 'Invalid payload' });
  const ann = repo.createAnnouncement(parsed.data as any); notify('announcement:new', ann); res.status(201).json(ann);
});
